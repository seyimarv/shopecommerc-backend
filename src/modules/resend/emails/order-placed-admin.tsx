import {
  ...
  function OrderPlacedAdminEmailComponent({ order }: OrderPlacedEmailProps) {
    return (
      <BaseEmailLayout previewText="You received an order">
        <Container className="p-6">
          <Heading className="text-2xl font-bold text-center text-gray-800">
            You received an order from {order.customer?.first_name || order.shipping_address?.first_name}
          </Heading>
        </Container>
        <OrderItems order={order} type="admin" />
        <OrderSummary order={order} />
      </BaseEmailLayout>
    )
  }
  
  export const orderPlacedAdminEmail = (props: OrderPlacedEmailProps) => (
